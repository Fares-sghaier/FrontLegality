...
const DashboardMessages = () => {
  ...
  const createNewConversation = async (groupTitle: string, userId: string, otherUserId: string) => {
    try {
      const response = await axios.post("http://localhost:5000/conversation/create-new-conversation", {
        groupTitle,
        userId,
        otherUserId
      });
  ...
  const getAllConversations = async (id: string) => {
    try {
      const response = await axios.get(`http://localhost:5000/conversation/get-all-conversation/${id}`);
  ...
  const updateLastMessage = async (id: string, lastMessage: string, lastMessageId: string) => {
    try {
      const response = await axios.put(`http://localhost:5000/conversation/update-last-message/${id}`, {
        lastMessage,
        lastMessageId
      });
      // Gérer la réponse et mettre à jour l'état de la conversation si nécessaire
  ...
  const sendMessage = async (conversationId: string, text: string, sender: string) => {
    try {
      const response = await axios.post("http://localhost:5000/conversation/message/create-new-message", {
        conversationId,
        text,
        sender
      });
  ...
  const getAllMessages = async (id: string) => {
    try {
      const response = await axios.get(`http://localhost:5000/conversation/message/get-all-messages/${id}`);
      ...
      try {
        if (user) {
          const response = await axios.get(
            `http://localhost:5000/conversation/get-all-conversation-user/${user.id_user}`,
            {
    ...
    const getMessage = async () => {
      try {
        if (currentChat && currentChat.id_user) {
          const response = await axios.get(
            `http://localhost:5000/conversation/message/get-all-messages/${currentChat.id_user}`
          );
    ...
    try {
      if (newMessage !== "") {
        const res = await axios.post(
          `http://localhost:5000/conversation/message/create-new-message`,
          message
        );
  ...
  const updateLastMessageForImage = async () => {
    try {
      await axios.put(
        `http://localhost:5000/conversation/update-last-message/${currentChat?.id_user}`,
        {
...
const MessageList = ({
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/user/user-info/${otherUserId}`);
