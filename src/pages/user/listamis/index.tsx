...
const FriendsList = () => {
  ...
  useEffect(() => {
    const fetchFriends = async () => {
      try {
        if (user) {
          const response = await axios.post('https://legality-back1-production.up.railway.app/users/friends', { id_user: user.id_user });
  ...
  const fetchMessages = async (friendId: string) => {
    try {
      const response = await axios.post('https://legality-back1-production.up.railway.app/messages/get-messages-by-participants', {
  ...
  const sendMessage = async (friendId: string, text: string) => {
    try {
      ...
      const response = await axios.post('https://legality-back1-production.up.railway.app/messages/create-new-message', {
        sender: user?.id_user,
  ...
  const getLastMessage = async (userId: string, friendId: string) => {
    try {
      const response = await axios.post('https://legality-back1-production.up.railway.app/messages/get-last-message', {
