...
const UserInvitationsPage = ( { fetchPendingInvitations }) => {
  ...
  useEffect(() => {
    const fetchInvitations = async () => {
      try {
        if (user) {
          const response = await axios.post('https://legality-back1-production.up.railway.app/users/invitations', { id_user: user.id_user });
  ...
  const handleAcceptInvitation = async (projectId: number, sendingUserId: number,inviteduserid: number) => {
    try {
      await axios.post("https://legality-back1-production.up.railway.app/projects/acceptinvitationbyuserinvited", {
  ...
  const handleRejectInvitation = async (projectId: number, sendingUserId: number) => {
    try {
      await axios.post("https://legality-back1-production.up.railway.app/projects/deleteinvitationbyuserinvited", {
        id_user: sendingUserId,
        id_project: projectId,
      });
