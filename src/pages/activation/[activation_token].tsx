...
const ActivationPage = () => {
  ...
  useEffect(() => {
    const activateAccount = async () => {
      ...
      if (activation_token) {
        try {
          const response = await axios.post('http://localhost:5000/users/activation', { activation_token });
  ...
  useEffect(() => {
    const checkTokenExpiration = async () => {
      try {
        await axios.post('http://localhost:5000/users/checktokenexpirationactivation', { token }); // Endpoint pour vérifier l'expiration du token
      } catch (error) {
