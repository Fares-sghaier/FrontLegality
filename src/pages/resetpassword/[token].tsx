...
const ResetPassword = () => {
  ...
  useEffect(() => {
    const checkTokenExpiration = async () => {
      try {
        await axios.post('http://localhost:5000/users/checktokenexpiration', { token }); // Endpoint pour vérifier l'expiration du token
      } catch (error) {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/users/resetpassword', { token, password });
